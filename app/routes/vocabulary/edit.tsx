...
import type { WordType, WordRequest, WordResponse, ApiError } from '~/types'
import { getWord, updateWord } from '~/api/client'
...
export function meta() {
  return [
    { title: 'Edit Word – Band Pilot' },
    { name: 'description', content: 'Edit an existing word in your IELTS vocabulary list' },
  ]
}
...
export default function EditWord() {
  ...
  // Form field state (pre-filled on load)
  ...
  useEffect(() => {
    ...
    getWord(id)
      .then((data: WordResponse) => {
        ...
        setWordType(data.type ?? '')
        setMeaning(data.meaning)
        setExamples(data.examples.length > 0 ? data.examples : [''])
      })
      .catch((err: ApiError) => {
        if (cancelled) return
        if (err.status === 404) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    await handleApiCall(() => updateWord(id!, payload), {
      onSuccess: () => {
  ...
  if (fetchLoading) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-8" aria-busy="true" aria-label="Loading word for editing">
        <div className="mb-4 h-5 w-48 animate-pulse rounded bg-gray-200" aria-hidden="true" />
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm space-y-5">
          <div className="h-9 w-56 animate-pulse rounded bg-gray-200" aria-hidden="true" />
          <div className="h-10 w-full animate-pulse rounded bg-gray-100" aria-hidden="true" />
          <div className="h-10 w-full animate-pulse rounded bg-gray-100" aria-hidden="true" />
          <div className="h-10 w-full animate-pulse rounded bg-gray-100" aria-hidden="true" />
          <div className="h-20 w-full animate-pulse rounded bg-gray-100" aria-hidden="true" />
          <div className="h-10 w-full animate-pulse rounded bg-gray-100" aria-hidden="true" />
        </div>
      </div>
    )
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-8">
        <nav className="mb-3 flex items-center gap-2 text-sm text-gray-500">
          <Link to="/vocabulary" className="hover:text-gray-700 focus:outline-none focus:underline">
            Vocabulary List
          </Link>
          <span aria-hidden="true">›</span>
          <span className="text-gray-900">Edit Word</span>
        </nav>
        <div
          role="alert"
        ...
        <div className="mt-4">
          <Link to="/vocabulary" className="text-sm font-medium text-blue-600 hover:underline focus:outline-none">
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Edit Word</h1>
      </div>
